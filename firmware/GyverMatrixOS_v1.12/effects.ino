// эффекты
#define effectScaleParam 50
#define effectScaleParam2 0
#define effectContrast 192
#define effectSpeed 192
...
void lightBallsRoutine() {
  if (loadingFlag) {
    ...
    BorderWidth = 0;
    USE_SEGMENTS_PAINTBALL = effectScaleParam2;
  }
  ...
  uint8_t blurAmount = map(globalBrightness, 32,255, 65,91);
  uint8_t actualBrightness = map(globalBrightness, 32,255, 125,250);
  blur2d(leds, WIDTH, HEIGHT, blurAmount);

  // The color of each point shifts over time, each at a different speed.
  uint32_t ms = millis();

  uint8_t  cnt = map8(255-effectScaleParam,1,4);  // 1..4 шариков
  ...
  if (USE_SEGMENTS_PAINTBALL != 0) {
    ...
    for (uint8_t ii = 0; ii < seg_num; ii++) {
      ...
      uint8_t fade_step = map8(globalBrightness, 1, 15);
...
void swirlRoutine() {
  if (loadingFlag) {
    ...
    USE_SEGMENTS_SWIRL = effectScaleParam2;
  }
  ...
  float spd = (map8(255-globalBrightness, 50, 100) / 100.0) / (USE_SEGMENTS_PAINTBALL != 0 ? 1 : (float)seg_num);
...
void rainbowRoutine(uint8_t rainbow_type) {
  if (loadingFlag) {
    loadingFlag = false;
    modeCode = MC_RAINBOW;  
    FastLED.clear();  // очистить
...
void rainbowDiagonal() {
  ...
  for (uint8_t x = 0; x < WIDTH; x++) {
    for (uint8_t y = 0; y < HEIGHT; y++) {
      ...
      CRGB thisColor = CHSV((uint8_t)(hue + dx * (float)(255 / maxDim)), 255, globalBrightness);
...
void rainbowHorizontal() {
  ...
  for (uint8_t j = 0; j < HEIGHT; j++) {
    CHSV thisColor = CHSV((uint8_t)(hue + j * map8(effectScaleParam,1,WIDTH)), 255, globalBrightness);
...
void rainbowVertical() {
  hue += 2;
  for (uint8_t i = 0; i < WIDTH; i++) {
    CHSV thisColor = CHSV((uint8_t)(hue + i * map8(effectScaleParam,1,HEIGHT)), 255, globalBrightness);
    for (uint8_t j = 0; j < HEIGHT; j++)
      drawPixelXY(i, j, thisColor);
  }
}
...
void rainbowRotate() {
  ...
  for(uint8_t y = 0; y < HEIGHT; y++) {
    ...
    for(uint8_t x = 0; x < WIDTH; x++) {
      pixelHue += xHueDelta8;
      leds[ XY(x, y)]  = CHSV( pixelHue, 255, globalBrightness);
    }
  }
}
...
void cyclonRoutine() {
  if (loadingFlag) {
    loadingFlag = false;
    modeCode = MC_CYCLON;
    USE_SEGMENTS_CYCLON = effectScaleParam2;
  ...
  uint8_t fader_param = map8(255 - effectScaleParam,0,5);
  fade_divider++;
...
void lightersRoutine() {
  ...
  for (uint8_t i = 0; i < map8(effectScaleParam,5,150); i++) {
    ...
    drawPixelXY(lightersPos[0][i] / 10, lightersPos[1][i] / 10, CHSV(lightersColor[i], 255, globalBrightness));
  }
}
...
void shadowsRoutine() {
  ...
  uint8_t  globalBrightness = getBrightnessCalculated(globalBrightness, effectContrast);
  ...
  for( uint16_t i = 0 ; i < NUM_LEDS; i++) {
    ...
    CRGB newcolor = CHSV( hue8, sat8, map8(bri8, map(globalBrightness, 32, 255, 32,125), map(globalBrightness, 32,255, 125,250))); 
    ...
    for (uint8_t r = 0; r < num_y; r++) {    
      ...
      if (block_sta[c][r] < 2) {
        ...
        uint8_t bri = fade_dir == 0
           ? map(fade_step, 0,FADE_IN_STEPS,  0, globalBrightness)
           : map(fade_step, 0,FADE_OUT_STEPS, globalBrightness,0);
...
void prizmataRoutine() {
  ...
  // Отрисовка режима происходит на максимальной скорости. Знеачение effectSpeed влияет на параметр BPM функции beatsin8
  uint8_t spd = map8(255-effectSpeed, 12, 64);   

  if (dir_mx == 0) {
    for (uint8_t x = 0; x < WIDTH; x++) {
      uint8_t y = beatsin8(spd + x, 0,HEIGHT-1);
      drawPixelXY(x, y, ColorFromPalette(RainbowColors_p, x * 7 + hue, globalBrightness));
    }
  } else {
    for (uint8_t y = 0; y < HEIGHT; y++) {
...
void munchRoutine() {
  ...
  for (uint8_t x = 0; x < minDimLocal; x++) {
    for (uint8_t y = 0; y < minDimLocal; y++) {
      CRGB color = (x ^ y ^ flip) < count ? ColorFromPalette(RainbowColors_p, ((x ^ y) << rnd) + generation, globalBrightness) : CRGB::Black;
...
void fire2Routine() {
  ...
  uint8_t cooling = map8(effectScaleParam, 70, 100);     
  
  // SPARKING: What chance (out of 255) is there that a new spark will be lit?
  // Higher chance = more roaring fire.  Lower chance = more flickery fire.
  uint8_t sparking = map8(effectScaleParam, 90, 150);
  ...
  for (uint8_t x = 0; x < WIDTH; x++) {
    ...
    for (uint8_t y = 0; y < HEIGHT; y++)
      nblend(leds[XY(x,y)], ColorFromPalette(HeatColors_p, ((noise3d[x][y]*0.7) + (noise3d[wrapX(x+1)][y]*0.3)), globalBrightness), fireSmoothing);
  }
}
